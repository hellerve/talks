...
# softmax: turn a row of scores into a probability distribution
def softmax(x):
    # shape in == shape out; each row sums to 1.
    # subtract the row max before exp() so it can't overflow.
    x = x - x.max(-1, keepdims=True)
    e = np.exp(x)
    return e / e.sum(-1, keepdims=True)


# self-attention: a weighted average of all positions
def attention(x, Wq, Wk, Wv):
    # x: (T, d).  project to Q, K, V; scores = Q @ K.T / sqrt(d_k) is (T, T);
    # return softmax(scores) @ V, back to (T, d).
    ...


# layer norm: standardise each position over its feature axis
def layer_norm(x, gamma, beta, eps=1e-5):
    # per row: (x - mean) / sqrt(var + eps), then * gamma + beta.
    ...


# the block: add each sublayer's output back to the running signal
def block(x, p):
    # x = x + attention(layer_norm(x, g1, b1), Wq, Wk, Wv)
    # x = x + mlp(layer_norm(x, g2, b2), W1, mb1, W2, mb2)
